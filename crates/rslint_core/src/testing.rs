...
macro_rules! rule_tests {
    // branch 1
    // pass rule, err and ok
    // dont want to use bool, still use expr for he bool value
    ($rule:expr,
    err: {
        $(
    ...
    // branch 2
    // pass rule, textmode val, err and ok
    ($rule:expr,
    $text_mode:expr,
    err: {
        $(
    ...
    ok: {
        ...
        rule_tests!(valid, invalid, $rule, $text_mode, err: { $($code),* }, ok: { $($ok_code),* });
    };

    // branch 3
    // pass okay name, error name, rule, error, okay
    (
    $ok_name:ident,
    $err_name:ident,
    $rule:expr,
    ...
    ok: {
        ...
        fn $err_name() {
            $(
               ...
               println!("text mode {}", $text_mode);

                let res = rslint_parser::parse_text($code, 0);
                let errs = $crate::run_rule(&$rule, 0, res.syntax(), true, &[], std::sync::Arc::from($code.to_string()));
                if errs.diagnostics.is_empty() {
                    panic!("\nExpected:\n```\n{}\n```\nto fail linting, but instead it passed (with {} parsing errors)", $code, res.errors().len());
                }
